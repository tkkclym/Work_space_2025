...
动规五部曲：dp的意义，状态转移方程，初始化，遍历顺序， 打印

到达m,n的位置 时有`dp[m][n]`条路经 注意是有多少种路径，而不是有多少步

`dp[m][n]=dp[m-1][n]+dp[m][n-1]`

初始化 `dp[1][0]=1;dp[0][1]=1`

遍历顺序，先m或者n无所谓的吧,因为都是向下或者向右走的，只要到目标位置就行
...
- 这个问题出现在哪里呢？初始化的时候，没有注意到，对于`dp[i][0]与dp[0][j]`都是1.就是第一行和第一列的路径个数都是1，**只有一种路径**（一直右走，或者一直下走），所以我这里的初始化不完全.【对于动态规划的初始化应该是有理有据的，而不是初始化成0试试，初始化成1试试这样，这这里初始化为1是因为到这个格子，只有一种路径】
...
- 还有一个问题就是循环的时候边界问题的处理，要小于m和n 不是小于等于，修改完之后就可以成功提交，还是比较简单的。
...
public:
    int uniquePaths(int m, int n) {
        ...
        for(int i=1;i<m;i++){
            for(int j=1;j<n;j++){
                dp[i][j]=dp[i-1][j]+dp[i][j-1];  递推公式 （代码实现顺序和思路顺序是不一致的,所以有时候看题解看不懂)
            }
        }
        return dp[m-1][n-1];
    }
...
这个数组创建的时候初始数组不用和之前走楼梯的一样扩容一个单位，为什么？

#### 解答：

因为这里的只需要在所在的位置存储到达该路径的个数即可，而爬楼梯的是从下标1开始的，下标0没有使用，所以要将数组扩容一个单位，并且最后返回的时候是返回的是 **下标为原数组长度的那个值，多想想这里**
...
using namespace std;
class Solution {
    vector<vector<int>>dp_p;
    public:
        int uniquePaths(int m, int n) {
            //初始化成员变量
            dp_p = vector<vector<int>>(m, vector<int>(n));
            vector<vector<int>>dp(m,vector<int>(n));
            for(int i=0;i<m;i++)dp[i][0]=1;
            for(int j=0;j<n;j++)dp[0][j]=1;//初始化
            for(int i=1;i<m;i++){
        ...
        void print(){
           int  m=dp_p.size();
           int n=dp_p[0].size();
            for(int i=0;i<m;i++)dp_p[i][0]=1;
            for(int j=0;j<n;j++)dp_p[0][j]=1;//初始化
            for(int i=0;i<m;i++){
                for(int j=0;j<n;j++){
                    cout<<dp_p[i][j]<<"   ";
                }
...
> 有时，直接看题解可能根本理解不了在说什么，因为思路和代码顺序可能是不同的

  代码随想录最后说了一句：动态规划题不适合一边写代码一边讲解，为什么，因为动态规划的思路和代码实现顺序是不一样的，突然想到为什么我们在看题解的时候很多时候看不懂是为啥呢？就是因为代码顺序和思路顺序是不一致的，所以直接看题解很艰难，**注意这个问题，在看题解代码之前最好思路是缕清的**

- 动态规划五步就是   理解dp含义   写出递推公式     进行初始化      找出遍历顺序

- 而代码中的顺序是   理解dp含义   进行初始化        找出遍历顺序    最后才写的递推公式
...
public:
    int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
        //将障碍物这个地方的dp[i]设置为0；刚好我也符合dp[i]的含义，到达这个点的路径为0个
        //找到障碍物的位置？dp[i][j]然后初始化的时候将这个位置的值初始化为0，在通过之前的做法往下算就行啊
        int m=obstacleGrid.size();
        int n=obstacleGrid[0].size();
        vector<vector<int>>dp(m,vector<int>(n));
        for(const auto&vec :obstacleGrid){
            for(auto tar: vec){
                if(tar==1){
                    dp[vec][tar]=0;//初始化1完成
                }
...
- 思路具有漏洞，没有完全覆盖范围，假如在第一行或者第一列的时候，只要有一个地方有 障碍，那么后面的都过不去，因为只能向下或者向右
- 错误的赋值方式，使用迭代器循环的时候不能使用数组下标赋值
- 不知道能够根据传入的二维数组直接通过下标访问元素。例：`if (obstacleGrid[0][0]==1||obstacleGrid[m-1][n-1]==1){return 0;}`
- 还有就是边界值的处理，在循环的时候，<=m了，应该是<m。当要返回的值是最后一个也就是[m-1]的时候，循环条件只要<m就可以。
...
其中在初始化的时候也可以这样写，把判断条件放在for循环的进入条件中（利用了for循环的性质 ）：
...
为什么？因为当不满足循环条件的时候，对整体代码而言，就可以向下进行了，而在此后面的元素都不会被赋值为1了在这里这也算for循环一个性质的巧妙使用，当然前提是，在dp声明的时候同时进行初始化即` vector<vector<int>> dp(m, vector<int>(n, 0));`
...
- 怎么想到使用二维数组的
- 二维数组代表什么含义  `dp[i][j]`中的i j又是代表什么 
- 对于给好的背包容量我们怎么处理？

解答：

`dp[i][j]`代表，从下标0~i之间任意选择物品放入容量为j的背包能够得到的最大价值（ i 就是包括下标i之前的所有物品，j 就是背包容量逐渐递增）

至于怎么想到用二维数组解题的？

给好的背包容量，使其逐渐递增到目标重量就行
...
情况1：不放物品i的情况是`dp[i-1][j]`，（时刻记住这个dp的含义）

情况2：放物品i的话就是 （背包容量-物品i的容量） 的最大价值 +物品i的最大价值    【意思就是之前没有i的空间的最大价值，加上i的价值，刚好i进来了，其他空间也装到最大价值了】



那既然我们要的是最大价值 所以将两种情况中的最大值找出来就可以`max(dp[i-1][j],dp[i-1][j-weight[j]+value[j] ])`
...
初始化的时候第一列因为物品背包容量为0.所以全部初始化为0

对第一行来说，当`dp[0][j]<weight[0]`的时候都应该为0，后面的能够装下物品0的初始化为物品0的价值

其他的任意初始化值都行

##### 第二种初始化方法：

dp数组全部初始化为0，然后第一行从下表为1的开始向右进行初始化，如果大于物品0的weight就换成物品0的weight

#### 遍历顺序

先遍历物品还是先遍历背包都是可以的，

因为在递推公式需要使用其值的时候，公式中的值都已经创建出来了。不影响公式的运算，所以遍历顺序都可以
...
因为我们有一个背包空间为0的情况，所以要将代表背包容量的值设置为n+1多一个元素位置。
...
int main(){
int m,bagstorage;//为物品种类，bagStorage为背包容量
cin>>m>>bagstorage;
vector<int>weight(m,0);//创建并初始化长度为m的数组

for(int &i:weight){
    cin>>i;//这里注意，用迭代器输入的时候，直接cin就可以
}
...
//然后创建一个二维数组 实现动态规划dp dp[i][j]的含义要明确 在0~i个物品中j容量下最大价值
//因为背包容量是从0开始的，占数组中一个位置，所以创建的二维数组应该是bagstorage+1
vector<vector<int>>dp(m,vector<int>(bagstorage+1,0));

//初始化
//直接初始化第一列即可
for(int i=1;i<=bagstorage;i++)
{
    if(weight[i]<=bagstorage){
        dp[0][i]=value[0];//这里是value[0]，你怎么赋值成value[i]了
    }
}
for(int i=1;i<m;i++){//先遍历物品
    for(int j=0;j<=bagstorage;j++){//遍历背包，就是现在是一个物品向右遍历
        if(j<weight[i]){//如果空间不够，就用之前的 @@@判断逻辑错误if(weight[j]>bagstorage) 改为 j<weight[i]
            dp[i][j]=dp[i-1][j];
        }else{//如果空间够了，那就看看之前的，与现在的那个价值更高
            dp[i][j]=max(dp[i-1][j],dp[i-1][j-weight[i]]+value[i]);
        }
    }
}
cout<<dp[m-1][bagstorage]<<endl;
...
使用的是一个一维数组进行滚动计算，每次都是使用上一层的值进行对本层的更新
...
vector<int>dp(bagStorage+1,0);  数组长度是多少，要长一点吗？初始化的时候很有讲究的，要初始化成非负数

for(int i=0;i<m;++i){
    for(int j=bagStorage;j>=weight[i];--j){
            dp[j]=max(dp[j],dp[j-weight[i]]+value[i]);     
    }
}
cout<<dp[bagStorage]<<endl;
return 0;
}
```

代码量很少，但是这里面的滚动时候的遍历顺序很重要，只看这一次不可能会的。这只是短时记忆在作祟。

1. 在内层循环遍历背包的时候，遍历顺序是倒叙的。你猜猜为什么倒叙，因为有递推公式可以得出，**倒序遍历是为了保证物品i只被放入一次！**。但如果一旦正序遍历了，那么物品0就会被重复加入多次！
2. 为什么说是说是会传入多次呢? 

以下是举例说明，加入背包大小为g（10），即将进来的物品重量为6，价值为9 

如果不装入该物体，那么应该就是g(10)【第二层的】=g(10)【第一层的】 也就是逻辑上将上一层复制下来的。

如果装该物体，那就是g(10-6) =g(4)+9 这个需要的**g(4)就很巧妙了**

为什么呢？**如果是按照正序的话，说明这个g(4)是属于本层更新过的**。逻辑上已经不属于上层的了



额逻辑滤清了，至于说拿了两次又是什么原因呢？本层的g(4)刚轮到的时候，拿了一次上层的g(4),而到g(10)又拿了一次本层的g(4),这就是两次，只是不是一层的东西
...
本题拆解之后就是：  背包容量相当于sum/2 我们要解决的问题就是把背包装满！！



首先就是dp数组的含义：容量为j的背包最大价值为dp[j]。

每个元素数值既是重量也是价值

背包装满也就是dp[sum/2]=sum/2

`dp[j]=max(dp[j],dp[j-weight[i]]+value[i])`

这里放不放dp[i]也是和上面那道题一样是由二维数组压缩而来。

所以这里的递推公式就是

`dp[j]=max(dp[j],dp[j-nums[i]]+nums[i])`

初始化的时候j=0的时候很好理解就是第一个背包值大小为0的时候，那价值dp[0]就是0

而后面的元素如何初始化，因为递推公式要求的是最大值，所有我们初始化那最小的非负整数0即可
