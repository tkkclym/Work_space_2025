...
```
	...
	1. 无限递归。 假如参数不是引用，而是传值，那么在调用赋值运算符的狮虎会触发拷贝函数，拷贝函数又可能调用赋值运算符，导致无限循环。
	1. 高效，传引用避免不必要的对象拷贝
	1. 语义正确性，赋值操作应该作用于现有对象，而不是创建一个新对象那。
...
取决于编译器和平台：通常存储在寄存器中，也可能是存储在栈上，但是注意this不是对象的一部分不占用对象的我内存空间。
