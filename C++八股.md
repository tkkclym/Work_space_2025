...
编译阶段分析代码结构确定每个类的继承关系（是否存在虚继承、菱形继承），然后根据分析结果确定每个类中虚基类表中的信息。

所以编译阶段做的事就是：1.分析类结构和继承关系 2.计算理论偏移值 3.生成虚基类表 4，添加指向虚基类表的指针

- 虚基类表中信息在**编译**时确定，在编译阶段编译器会处理类的定义，虚继承关系，确定虚基类表的结构，**计算理论偏移量信息，**并生成相应的构造函数调用代码，但不会创建实际对象和虚基类子对象
- 在运行阶段创建最终派生类对象的语句时，会按照特定顺序构造对象，其中包括虚基类子对象。假如是D构造的话，具体构造顺序就是虚基类构造，直接基类构造，最终派生类构造
...
​	**虚基类表是一个静态表格，其中包含以下信息：**
	...
	2. 构造函数调用信息：虚基类表中有可能提供虚基类构造函数入口地址，以便对象构造的时候正确初始化
	3. 析构函数调用信息：虚基类表中有可能有虚基类析构函数的入口地址，以便对象构造的时候正确初始化虚基类
	4. RTTI信息：某些编译器中可能包含运行时类型信息，用于支持Dynamic_cast等操作。
...
之前疑惑了他是咋知道哪些是菱形继承的最下面的派生类，哪些是菱形继承中间的类，编译阶段进行分析了。然后关于虚基类子对象的内容在这里：

https://www.doubao.com/thread/wca5c468456a2a8eb
...
4. **链接**：
   ...
   - 作用：能够将各个目标文件和库文件整合为一个完整可执行文件或者共享库，解决符号引用和地址分配问题，使程序能够在目标机器上正确执行
